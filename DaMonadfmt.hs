...
sortByKeys keys =
    sortBy
        ( compare
            `on` (\x -> if x == Nothing then Just 100000000 else x)
                . flip lookup (zip keys [0 ..])
        )
...
-- stable (ms, ws) ws' = null ( do
--                              m <- ms
--                              w <- ws
--                              guard $ pid m `elem` (snd . head) (filter (\ x -> fst x == aid w) (S.toList ws'))
--                              guard $ any  (\wm -> let thew =(\(Just a) -> a) (find (\x -> aid x == fst wm) ws)
...
--                              return 1 )
--                            &&
--                              null [1| m <- ms,  pid m `notElem` (foldr union [] (map enrol ws')),
--                                    any (==True) (map  (\wm -> (pid m) `elem` (take (quota wm) (sortByKeys (prio wm) ((pid m):(enrol wm)))) )
...
main = do
    ...
    let result = effiDA (S.fromList ms, S.fromList ws) -- head results
